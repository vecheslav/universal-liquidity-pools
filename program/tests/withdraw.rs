...
async fn setup() -> (ProgramTestContext, TestPoolMarket, TestPool, User) {
    ...
    let user = add_user(&mut context, &test_pool, 9999 * EXP)
        .await
        .unwrap();
...
async fn success() {
    ...
    assert_eq!(get_token_balance(&mut context, &user.destination).await, 50);
    assert_eq!(
...
async fn success_with_rate() {
    let (mut context, test_pool_market, test_pool, user) = setup().await;
    let start_source_balance = get_token_balance(&mut context, &user.source).await;
    ...
    let destination_balance = get_token_balance(&mut context, &user.destination).await;
    // Around 166
    ...
    assert_eq!(get_token_balance(&mut context, &user.destination).await, 0);
    assert_eq!(
